...
{
  echo "Usage: {$argv[ 0 ]} <URL> <check source> [accept redirects]\n";
...
do
{
  ...
  curl_setopt( $ch, CURLOPT_URL, "http://api.wheresitup.com/v3/retrieve/$jobID" );
...
function submitJob( $uri, $server, $services )
{
  ...
  curl_setopt( $ch, CURLOPT_URL, 'http://api.wheresitup.com/v3/submit' );
  curl_setopt( $ch, CURLOPT_HTTPHEADER, array( "Auth: Bearer " . clientID . ' ' . token ) );
